...
export default class BaseNode{
    ...
    addChild(nodeType){
        var parentLeft=parseInt(this.nodeContainer.style.left)+3;
        var parentTop=parseInt(this.nodeContainer.style.top)+3;
        var conn=new Connection(this.context,nodeType,this);
        var childrenCount=this.children.length;
        var w=this.nodeContainer.offsetWidth,h=this.nodeContainer.offsetHeight;
        ...
        if(childrenCount>0){
            if(this.parentNode){
                var newNodeHeight=conn.node.nodeContainer.offsetHeight+15;
                var parentChildren=this.parentNode.children;
                var pos=parentChildren.indexOf(this.connection);
        ...
        if(this.context.lastLeftNode){
            var left=parseInt(conn.node.nodeContainer.style.left);
            var oldLeft=parseInt(this.context.lastLeftNode.nodeContainer.style.left);
            if(left>oldLeft){
        ...
        if(this.context.lastBottomNode){
            var top=parseInt(conn.node.nodeContainer.style.top);
            var oldTop=parseInt(this.context.lastBottomNode.nodeContainer.style.top);
            if(top>oldTop){
    ...
    resizeNode(){
        ...
        var nodeHeight=this.nodeContainer.offsetHeight+15;
        ...
        var nodeWidth=this.nodeContainer.offsetWidth;
    ...
    resetCanvasSize(){
        if(this.context.lastLeftNode){
            var left=parseInt(this.context.lastLeftNode.nodeContainer.style.left);
            this.context.container.style.width=(left+this.context.lastLeftNode.nodeContainer.offsetWidth+30)+"px";
        }
        if(this.context.lastBottomNode){
            var top=parseInt(this.context.lastBottomNode.nodeContainer.style.top);
    ...
    resetItemWidth(difWidth){
        for(var i=0;i<this.children.length;i++){
            ...
            var left=parseInt(node.nodeContainer.style.left)+difWidth;
    ...
    resetParentNodePosition(){
        ...
        if(this.children.length>1){
            ...
            var firstNodeTop=parseInt(firstNodeContainer.style.top);
            var lastNodeTop=parseInt(lastNodeContainer.style.top)+lastNodeContainer.offsetHeight;

...
export default class RuleForgePropertiesPanel extends Component {
    ...
    openScriptEditor(currentValue, title, lintType, callback) {
        this.setState({
    ...
    renderField(label, value, onChange, placeholder, type) {
        return (
    ...
    renderFileField(label, value, onValueChange, placeholder) {
        return (
            <div className="rf-prop-group">
                <label>{label}</label>
    ...
    renderNameField() {
        const bo = this.getBusinessObject();
        return this.renderField('名称', bo.name, (e) => this.updateName(e), '节点名称');
    }

    renderEventBeanField() {
        return this.renderField('事件Bean', this.getExtensionAttr('eventBean'),
            (e) => this.setExtensionAttr('eventBean', e.target.value), 'Spring Bean ID（可选）');
    }
        ...
        return (
            <>
                {this.renderFileField('规则文件', this.getExtensionAttr('file'),
                    (e) => this.setExtensionAttr('file', e.target.value), '选择或输入规则文件路径')}
                {this.renderField('项目', this.getExtensionAttr('project'),
                    (e) => this.setExtensionAttr('project', e.target.value), '项目名称')}
                {this.renderField('版本', this.getExtensionAttr('version'),
                    (e) => this.setExtensionAttr('version', e.target.value), '留空使用最新版本')}
            </>
        );
    }
        ...
        return (
            <>
                {this.renderField('动作Bean', this.getExtensionAttr('bean'),
                    (e) => this.setExtensionAttr('bean', e.target.value), '实现FlowAction接口的Spring Bean ID')}
                {this.renderField('方法名', this.getExtensionAttr('method'),
                    (e) => this.setExtensionAttr('method', e.target.value), '方法名称')}
            </>
        );
    }
    ...
    openRuleEditDialog(rulesList, idx, updateCallback) {
        ...
        const nameInput = container.querySelector('.rf-rule-name');
        const fileInput = container.querySelector('.rf-rule-file');
        const versionInput = container.querySelector('.rf-rule-version');
        const eventBeanInput = container.querySelector('.rf-rule-event-bean');

    ...
    componentDidMount() {
        event.eventEmitter.on(event.OPEN_BATCH_TEST_DIALOG, (config) => {
            this.setState({
                ...
                data: config.data,
                files: config.files,
                sessionId: config.sessionId || window._importSessionId || null,
                progress: 0,
                status: '',
                totalRows: 0,
                errorCount: 0,
        ...
        if (polling || status === 'RUNNING') {
            ...
            body = (
                <div style={{padding: '15px'}}>
                    ...
                    <button className="btn btn-success" onClick={() => document.getElementById(exportFormId).submit()}>

...
export default class VersionsDialog extends Component{
    constructor(props){
        super(props);
        this.state={
            visible: false,
    ...
    render(){
        ...
        // 知识包文件列表
        const resourceData = this.state.rowData && this.state.rowData.resourceItems || []
        ...
        const body=(
            <div>
                ...
                <div className="form-group" style={{display: 'flex'}}>
                    ...
                    <div style={{flex: 1}}>
                        <Grid headers={resourceGridHeaders} dispatch={dispatch} rows={resourceData} rowClick={(rowData) => {
                            setTimeout(() => {
                                action.getPackageDiffList(this.props.project, rowData.path,(list) => {
                                    list.forEach(item => {
        ...
        const buttons=[
            {
                ...
                icon:'fa fa-floppy-o',
                click:function () {
                    let associatedFiles = this.state.diffList.map(item => `${item.path}:${item.targetVersion}`)

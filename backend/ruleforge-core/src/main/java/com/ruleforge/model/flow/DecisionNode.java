...
public class DecisionNode extends BindingNode {
    ...
    private void doCriteria(FlowContext context, FlowInstance instance) {
        ...
        if (to == null) {
            // todo
            List<ReteNode> reteNodeList = ((DecisionNode) instance.getCurrentNode()).getKnowledgePackageWrapper().getAllNodes();
            if (reteNodeList != null && !reteNodeList.isEmpty()) {
                for (ReteNode reteNode : reteNodeList) {
                    if (reteNode instanceof CriteriaNode) {
                        CriteriaNode criteriaNode = (CriteriaNode) reteNode;
                        if ("[eval]\"其余条件\"【等于】[字符]else".equals(criteriaNode.getCriteriaInfo())) {
                            TerminalNode terminalNode = ((TerminalNode) criteriaNode.getChildrenNodes().get(0));
                            VariableAssignAction variableAssignAction = (VariableAssignAction) terminalNode.getRule().getRhs().getActions().get(0);
                            to = ((SimpleValue) variableAssignAction.getValue()).getContent();
                            break;
                        }
                    }
                }
